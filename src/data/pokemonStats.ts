// Base stats for Pokemon (Gen 1-7)
// Format: [HP, Attack, Defense, Sp.Atk, Sp.Def, Speed]
...
// Stats database - storing as array for compactness: [hp, atk, def, spa, spd, spe]
const STATS_DATA: Record<number, number[]> = {
  // Gen 1
  1: [45, 49, 49, 65, 65, 45],
  2: [60, 62, 63, 80, 80, 60],
  ...
  // Gen 3-7 stats simplified (using averages for common Pokemon)
};

// Generate default stats for Pokemon without specific data
const generateDefaultStats = (id: number): number[] => {
  // Base on rarity/legendary status
  if (id >= 144 && id <= 151) return [90, 100, 90, 100, 90, 100]; // Legendary
  if (id >= 243 && id <= 251) return [100, 100, 100, 100, 100, 100]; // Legendary
  // Default common Pokemon stats
  return [50 + (id % 30), 55 + (id % 25), 50 + (id % 20), 55 + (id % 25), 50 + (id % 20), 55 + (id % 25)];
};

export const getBaseStats = (pokemonId: number): BaseStats => {
  const stats = STATS_DATA[pokemonId] || generateDefaultStats(pokemonId);
...
export const getStatColor = (value: number): string => {
  if (value >= 150) return 'hsl(280 70% 50%)'; // Purple - exceptional
  if (value >= 120) return 'hsl(340 80% 50%)'; // Pink - very high
  if (value >= 100) return 'hsl(145 65% 42%)'; // Green - high
  if (value >= 80) return 'hsl(50 90% 50%)'; // Yellow - good
  if (value >= 60) return 'hsl(25 85% 55%)'; // Orange - average
  return 'hsl(0 84% 60%)'; // Red - low
};

// Get stat bar width percentage (max stat is 255)
export const getStatPercent = (value: number): number => {
